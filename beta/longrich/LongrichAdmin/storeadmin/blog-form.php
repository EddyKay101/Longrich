...
$message = "";
	if(isset($_POST['title'])){
		...
		if($entryMatch > 0){
			echo'Sorry you tried to place a duplicate into the system,<a href="entries-html.php">click here</a>';
			exit();
		}
		
		//Insert this product into the database
		$query = "INSERT INTO entry(title,entry_text,date_created) VALUES('$title','$entrytext',now())";
		$result = $conn->query( $query );
		$pid = $conn->insert_id;
	if ( !$result )die( "Database access failed: " . $conn->error );
	...
	<div id="wrapper">
			...
			<form action="blog-form.php" enctype="multipart/form-data" name="myForm" id="myForm" method="post">
